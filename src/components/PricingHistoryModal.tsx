...
const fieldLabel = (f: string) =>
  f === 'price' ? 'Wholesale' : f === 'msrp' ? 'MSRP' : f;
...
export default function PricingHistoryModal({
  ...
  const [edits, setEdits] = useState<PricingEditEntry[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/data/pricing-history?pricingId=${encodeURIComponent(pricingId)}`);
      ...
      <div className="relative bg-surface rounded-xl shadow-2xl border border-primary max-w-2xl w-full mx-4 overflow-hidden max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between gap-3 p-5 border-b border-primary">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-text-primary">Price Edit History</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {!error && edits === null && <p className="text-sm text-text-muted">Loading…</p>}
          {edits && edits.length === 0 && (
            <p className="text-sm text-text-muted">No edits recorded for this price yet.</p>
          )}
          {edits && edits.length > 0 && (
              ...
              <tbody>
                {edits.map((e) => (
                  <tr key={e.id} className="border-b border-primary/50 last:border-0">
                    ...
                    <td className="py-2 pr-3 text-text-secondary">{fieldLabel(e.field)}</td>
