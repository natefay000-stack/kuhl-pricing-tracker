...
/**
 * Pre-aggregated invoice data for the Invoice Month view.
 *
 ...
 *   - Top 500 styles
 *   - Top 500 customers
 *   - Filter option lists (seasons, customer types, customers, colors)
 *
...
export async function GET() {
  try {
    ...
    const yearMonth = await prisma.$queryRawUnsafe<
      Array<{ y: number; m: number; cnt: bigint; net: number }>
    >(`
      SELECT
    ...
    const topStyles = await prisma.$queryRawUnsafe<
      Array<{ styleNumber: string; styleDesc: string | null; shipped: number; returned: number; net: number }>
    >(`
      SELECT
        "styleNumber",
    ...
    const topCustomers = await prisma.$queryRawUnsafe<
      Array<{ customer: string; shipped: number; returned: number; net: number }>
    >(`
      SELECT
        "customer",
        COALESCE(SUM("shippedAtNet"),0)::float AS shipped,
    ...
    return NextResponse.json({
      success: true,
      yearMonth: yearMonth.map((r) => ({ y: r.y, m: r.m, count: Number(r.cnt), net: r.net })),
